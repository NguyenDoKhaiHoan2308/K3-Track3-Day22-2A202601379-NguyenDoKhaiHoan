...
def evaluate(config: Annotated[Path, typer.Option("--config")]) -> None:
    ...
    chosen_scores = [deterministic_response_score(example.chosen) for example in examples]
    rejected_scores = [deterministic_response_score(example.rejected) for example in examples]
